...
public class OpenGL {
    ...
    public static void shaderSetup() {
        try {
            Shader vertex_shader_vc = Shader.loadShader(GL20.GL_VERTEX_SHADER, "src/main/java/net/revenant/opengl/VERTEX_SHADER_VC.glsl");
            Shader fragment_shader_vc = Shader.loadShader(GL20.GL_FRAGMENT_SHADER, "src/main/java/net/revenant/opengl/FRAGMENT_SHADER_VC.glsl");
            Shader vertex_shader_vct = Shader.loadShader(GL20.GL_VERTEX_SHADER, "src/main/java/net/revenant/opengl/VERTEX_SHADER_VCT.glsl");
            Shader fragment_shader_vct = Shader.loadShader(GL20.GL_FRAGMENT_SHADER, "src/main/java/net/revenant/opengl/FRAGMENT_SHADER_VCT.glsl");
            ...
            if(UNI_VC_MODEL != -1) {
                GL20.glUniformMatrix4(UNI_VC_MODEL, false, pureIdentity.getBuffer());
            }

            UNI_VC_VIEW = GL20.glGetUniformLocation(shaderProgram_VC.id, "view");
            if(UNI_VC_VIEW != -1) {
                GL20.glUniformMatrix4(UNI_VC_VIEW, false, pureIdentity.getBuffer());
            }

            UNI_VC_PROJECTION = GL20.glGetUniformLocation(shaderProgram_VC.id, "projection");
            if(UNI_VC_PROJECTION != -1) {
                GL20.glUniformMatrix4(UNI_VC_PROJECTION, false, identityProjection.getBuffer());
            }
            // ================ shader uniform setup ====
            ...
            if(UNI_VCT_MODEL != -1) {
                GL20.glUniformMatrix4(UNI_VCT_MODEL, false, pureIdentity.getBuffer());
            }

            UNI_VCT_VIEW = GL20.glGetUniformLocation(shaderProgram_VCT.id, "view");
            if(UNI_VCT_VIEW != -1) {
                GL20.glUniformMatrix4(UNI_VCT_VIEW, false, pureIdentity.getBuffer());
            }

            UNI_VCT_PROJECTION = GL20.glGetUniformLocation(shaderProgram_VCT.id, "projection");
            if(UNI_VCT_PROJECTION != -1) {
                GL20.glUniformMatrix4(UNI_VCT_PROJECTION, false, identityProjection.getBuffer());
            }
    ...
    public static void glSetMatrix(int matrixId, Matrix4f matrix) {
        GL20.glUniformMatrix4(matrixId, false, matrix.getBuffer());
    }
